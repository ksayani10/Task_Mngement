...
r.get("/tasks/my", requireAuth, async (req, res) => {
  try {
    ...
    const tasks = await db.task.findMany({
      // Use relation filter instead of unknown FK scalar
      where: { assignee: { is: { id: uid } } },
      orderBy: { id: "asc" },
      include: { assignee: { select: { id: true, name: true } } },
    });
...
/**
 * POST /tasks
 * Create a task (member creates for THEMSELVES)
 * body: { projectId: number, title: string, dueDate?: string | null }
 */
r.post("/tasks", requireAuth, async (req, res) => {
  try {
    const uid = req.user!.uid;
    ...
    const task = await db.task.create({
      data: {
        ...
        // connect via relations (schema-agnostic)
...
r.patch("/tasks/:id", requireAuth, async (req, res) => {
  try {
    ...
    const isOwner = task.assignee?.id === uid;
    const isAdmin = req.user!.role === "admin";
    if (!isOwner && !isAdmin) return res.status(403).json({ error: "Forbidden" });
...
/**
 * PATCH /tasks/:id/assign
 * Admin-only: assign any task to any user
 * body: { assigneeUserId: number }
 */
r.patch("/tasks/:id/assign", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    if (!assigneeUserId) {
      return res.status(400).json({ error: "assigneeUserId required" });
    }

    const updated = await db.task.update({
      where: { id },
      data: { assignee: { connect: { id: Number(assigneeUserId) } } },
      include: { assignee: { select: { id: true, name: true } } },
    });
